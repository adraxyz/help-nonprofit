<template>
  <section id="fivepermille-section-4" class="padding-level-3">
    ...
    <div class="text-center">
      <div>
        <span class="text-subtitle input-title d-block" v-html="baseText ? baseText.title : ''"/>
      ...
      <div class="text-title result text-center" v-show="five_x_1000">
        <span>{{baseText ? baseText.subtitle : ''}}</span>
...
<script>
  export default {
    ...
    computed: {
      baseText() {
        return this.data ? this.data.texts.find(t => t.order === 0) : null
      }
    },
    methods: {
      calc5x1000() {
